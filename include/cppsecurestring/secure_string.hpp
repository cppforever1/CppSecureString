...
namespace cppsecurestring
{
    ...
    public:
        ...
        {
            other.with_plaintext([this](string_view_type value)
                                 { assign(value); });
        }
              ...
              key_(other.key_), nonce_(other.nonce_), key_locked_(detail::lock_memory(key_.data(), key_.size()))
        {
            ...
            if (other.key_locked_)
                detail::unlock_memory(other.key_.data(), other.key_.size());
        ...
        ~basic_secure_string()
        {
            ...
            if (key_locked_)
                detail::unlock_memory(key_.data(), key_.size());
        }
        ...
        basic_secure_string &operator=(string_view_type value)
        {
            assign(value);
            return *this;
        }
        ...
        {
            if (index >= size_)
                throw std::out_of_range("basic_secure_string::at");
        ...
        friend bool operator==(const basic_secure_string &left, const basic_secure_string &right)
        {
            if (left.size_ != right.size_)
                return false;
            return left.with_plaintext([&right](string_view_type value)
                                       { return right == value; });
        }
        ...
        friend std::basic_istream<CharT, Traits> &operator>>(std::basic_istream<CharT, Traits> &input, basic_secure_string &value)
        {
            ...
            if (input.good() || input.eof())
                value.assign(string_view_type(buffer.data(), buffer.size()));
            return input;
        }
    ...
    private:
        ...
        {
            if (index >= size_)
                throw std::out_of_range("basic_secure_string::set");
            mutate([=](string_type &plain)
                   { plain[index] = value; });
        }
        template <typename Function>
