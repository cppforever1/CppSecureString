...
#else
    if (::mlock(reinterpret_cast<void*>(address), size) != 0) return false;
...
void store32(std::byte* output, std::uint32_t value) noexcept {
    for (int index = 0; index < 4; ++index) output[index] = std::byte(value >> (index * 8));
}
...
void secure_random(std::span<std::byte> output) {
    ...
    while (completed < output.size()) {
        ...
        if (result > 0) completed += static_cast<std::size_t>(result);
        else if (result < 0 && errno != EINTR) throw std::runtime_error("getrandom failed");
    }
#endif
}
