...
namespace
{
    ...
    void test_copy_move_and_callback()
    {
        ...
        const auto length = moved.with_plaintext([](std::string_view value)
                                                 { return value.size(); });
        expect(length == 0, "moved-from object remains valid");
        const auto assigned_length = assigned.with_plaintext([](std::string_view value)
                                                             { return value.size(); });
...
int main()
{
    ...
    if (failures == 0)
        std::cout << "All CppSecureString tests passed\n";
